/*
 * package reportConfig; import java.util.Calendar; import java.util.Date;
 ...
 * org.testng.Reporter; import org.testng.xml.XmlSuite;
 * 
 ...
 * 
 * 
 * public class ExtentReportListener implements IReporter { private
 * ExtentReports extent; private ExtentTest test;
 * 
 * @Override public void generateReport(List<XmlSuite> xmlSuites, List<ISuite>
 * suites, String outputDirectory) { extent = new
 * ExtentReports(System.getProperty("user.dir") +
 * "/ExtentReportV2/ExtentReport.html", true);
 * 
 * for (ISuite suite : suites) { Map<String, ISuiteResult> result =
 * suite.getResults();
 * 
 * for (ISuiteResult r : result.values()) { ITestContext context =
 * r.getTestContext(); buildTestNodes(context.getFailedTests(), LogStatus.FAIL);
